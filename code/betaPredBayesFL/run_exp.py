...
"""
We have used the code base of Hasan et al. (2024) for the competing methods in the 
Real-life classification setting using Bayesian Neural Networks from Hasan et al. (2024).
Their code is publicly available at: https://github.com/hasanmohsin/betaPredBayesF.
We have only modified this code base, contained within this folder, to set the settings 
equal to ours. 
Sample scipts to run the experiments are provided in the 'sample_experiments' folder. 
Please modify the '--device' argument to whichever device you are using, e.g. 'cpu' or 'cuda:0'.
"""
##################################################################
...
def main(args):
    ...
    logger = open(fname+".txt", 'a')
    logger.write(f"{fname} with: \n\
                - Number of Clients: {args.num_clients}\n\
                - Random contamination: {args.contamination_type_random} ({args.contamination_rate}, {args.contamination_seed})\n\
                - Optimiser: {args.optim_type} (lr: {args.lr}, server iters: {args.num_rounds})\n\n\
                RESULTS:\n")
    ...
    if args.dataset == "mnist":
        trainloader, valloader, train_data  = datasets.get_mnist(use_cuda, args.batch_size, \
        contamination_rate = args.contamination_rate, random_cont= args.contamination_type_random, seed=args.contamination_seed, get_datamat=True)
        inp_dim = 28*28
        ...
        trainloader, valloader, train_data  = datasets.get_emnist(use_cuda, args.batch_size, \
        contamination_rate = args.contamination_rate, random_cont= args.contamination_type_random, seed=args.contamination_seed, get_datamat=True)
        inp_dim = 28*28
    elif args.dataset == "f_mnist":
        trainloader, valloader, train_data  = datasets.get_fashion_mnist(use_cuda, args.batch_size, \
        contamination_rate = args.contamination_rate, random_cont= args.contamination_type_random, seed=args.contamination_seed, get_datamat=True)
        inp_dim = 28*28
    ...
    elif args.net_type == "fc1h":
        base_net = models.LinearNet1Hidden(inp_dim=inp_dim, num_hidden = 200, out_dim=out_dim)
        base_net = base_net.to(device)
...
if __name__ == '__main__':
    ...
    #for contamination of training data
