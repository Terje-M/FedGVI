...
def contaminate_labels(data, epsilon, seed=42, random=True):
    ...
    if random:
        random_contamination(data, corrupted_indices, seed=seed)
    else:
        deterministic_contamination(data, corrupted_indices, seed=seed)
    
    return corrupted_indices

def random_contamination(data, indices, seed=42):
    labels = list(range(int(min(data)), int(max(data))+1))
    ...
    for idx in indices:
        ...
        other_labels.remove(int(data[idx]))
...
def get_mnist(use_cuda, batch_size, contamination_rate = 0.1, random_cont= True, seed=42 ,get_datamat = False):
    # data augmentation
    ...
    true_train_labels = copy.deepcopy(train_data.targets)
    contaminated_indices = contaminate_labels(train_data.targets, contamination_rate, seed=seed, random=random_cont)
    
    for i in contaminated_indices:
        assert train_data.targets[i] != true_train_labels[i], "Contamination has incurred an ERROR!"
...
# 10,000 validation points
def get_emnist(use_cuda, batch_size, contamination_rate = 0.1, random_cont= True, seed=42, get_datamat = False):
    # data augmentation
    transform_train = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=(0.1307,), std=(0.3081,))
...
# _ train points, 
# _ validation points
def get_fashion_mnist(use_cuda, batch_size, contamination_rate = 0.1, random_cont= True, seed=42, get_datamat = False):
    # data augmentation
    transform_train = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=(0.1307,), std=(0.3081,))
