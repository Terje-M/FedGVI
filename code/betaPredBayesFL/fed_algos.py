...
class FedAvg:
    ...
    def save_models(self):
        ...
        save_name = self.exp_id + "_seed_"+str(self.seed) + "_net-type_" + str() 
    ...
    def get_acc(self, net, valloader):
        if self.task == "classify":
            return utils.classify_acc(net, valloader, device=self.device)
        else:
    ...
    def train(self, valloader):
        for i in range(self.num_rounds):
            ...
            #nllhd = test_classify_nllhd(model = self.global_net, testloader=valloader, device = self.device, model_type= "single")
...
# BCM
#Implements PredBayes (Hasan et al., 2024) (no distillation step)
...
# Implements D BetaPredBayes (Hasan et al., 2024)
# same as F_MCMC but an additional distillation step is added
...
class Calibrated_PredBayes_distill(EP_MCMC):
    ...
    def train(self, valloader):
        ...
        #save to dict
        utils.write_result_dict_to_file(result=f_mcmc_acc, seed = self.seed, 
                                 file_name= self.save_dir + utils.change_exp_id(exp_id_src=self.exp_id, source_mode = "distill_f_mcmc", target_mode="f_mcmc") + "_seed_" + str(self.seed),
                                 type = "acc")
        ...
        utils.write_result_dict_to_file(result=prod_f_mcmc_acc, seed = self.seed, 
                                 file_name= self.save_dir + utils.change_exp_id(exp_id_src=self.exp_id, source_mode = "distill_f_mcmc", target_mode="product_f_mcmc") + "_seed_" + str(self.seed),
                                 type = "acc")
        ...
        utils.write_result_dict_to_file(result=mix_f_mcmc_acc, seed = self.seed, 
                                 file_name= self.save_dir + utils.change_exp_id(exp_id_src=self.exp_id, source_mode = "distill_f_mcmc", target_mode="mixture_f_mcmc") + "_seed_" + str(self.seed),
                                 type = "acc")
        ...
        #save to dict 
        utils.write_result_dict_to_file(result=ep_mcmc_acc, seed = self.seed, 
                                 file_name= self.save_dir + utils.change_exp_id(exp_id_src=self.exp_id, source_mode = "distill_f_mcmc", target_mode="ep_mcmc") + "_seed_" + str(self.seed),
                                 type = "acc")
        ...
        if self.task == "classify":
            ...
            #save to dict 
            utils.write_result_dict_to_file(result=fed_pa_acc, seed = self.seed, 
                                 file_name= self.save_dir + utils.change_exp_id(exp_id_src=self.exp_id, source_mode = "distill_f_mcmc", target_mode="fed_pa") + "_seed_" + str(self.seed),
                                 type = "acc")
